...
```{r }
luconames_base <- readxl::read_excel(
    "230707 rename Lu seed cos.xlsx"
)
fread2 <- function(x){
  data.table::fread(x, colClasses = 'character', data.table = FALSE  )
  
}
patent_pvp_combine <-  tibble(
   
   filename = list.files(
        "~/seed230814/pvp patent combine",
        pattern = "csv",
        full.names = TRUE
    ...
    mutate(
      dfcall = 
      purrr::map(filename, ~possibly(~fread2(.x),otherwise=NA)(.x)) 
    ) %>% 
    unnest()

#only those most that are for sure AOH1
patent_pvp_combine_filter <- patent_pvp_combine %>% 
  filter(
    (valueuse_ifcpc_A01H6 == 1)|(patent_type_orig == "PVP")
  ) 
```
...
lu_conames_out230705  <- patent_pvp_combine  %>% 
  select(
